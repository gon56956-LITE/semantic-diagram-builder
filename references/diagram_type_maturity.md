...
For production selection, use `references/diagram_type_readiness.md`. Maturity tracks implementation evidence; readiness tracks whether a type is safe for real deliverables and what authoring discipline it needs.
...
| `ontology_map` | Mature for ontology-style concept maps | Two concepts, a predicate diamond, cardinality labels, and an instance example. | Blueprint ontology map with concepts, attributes, relationship diamonds, instances, and bottom legend/about/rules/version panels. | Dense concept map with multiple concept rows, many predicate diamonds, multiple instances per concept, and bottom governance/metadata panels. | Concept cards are not ER table cards; instances remain subordinate; relationship diamonds reuse fixed-slot routing; instance links use staggered lanes when corridors repeat; bottom panels do not steal width from the concept canvas. |
| `capability_domain_map` | Mature for banded capability maps | Two levels, two columns, domain/capability items, and bottom usage panel. | Blueprint capability/domain map with objectives, domains, sub-domains, capabilities, enablers, and bottom panels. | Wide dense map with many columns, stacked sub-domains/capabilities, shared enablers, sparse dependency overlays, explicit lane offsets, and bottom QA panels. | Level labels and column labels remain readable; stacked cards do not overlap or crowd; sparse connectors stay in row gaps or offset corridors; bottom panels render as readable blueprint blocks without invading map items. |
| `relationship_matrix` | Beta reference implementation | Four same-entity rows/columns with direct, indirect, dependency, static focus detail, and empty cells. | Blueprint companion view with reference-only primary preview, same-entity matrix, focus details, compact summary/distribution bars, and top-connected ranking. | 11-entity high-connectivity matrix with sparse dependencies, focus dependency detail, and compact bottom support panels. | Matrix cells stay readable; row/column labels do not overflow; static focus detail does not look interactive; support panels sit below the matrix and do not dominate it; accents remain sparse and blueprint-first. |
...
| Ontology map degenerates into an ER table. | `ontology_map` | Use concept cards without PK/FK badges, instance cards, datatype rows, and bottom panels. |
| Ontology support panels steal width from the concept core. | `ontology_map` | Render support panels below the concept canvas rather than reserving side gutters. |
| Ontology instance links reuse the same lane. | `ontology_map` | Use `instances[].lane_offset` when multiple examples share a concept or route through the same long corridor. |
...
| Dense graph becomes unreadable but the matrix turns into a dashboard. | `relationship_matrix` | Keep the primary network as reference-only, emphasize the row/column matrix, and keep auxiliary details in compact bottom panels. |
| Static matrix looks interactive because one cell is highlighted. | `relationship_matrix` | Use the bottom focus-detail panel for explanation; do not draw selected-cell markers in the matrix grid. |
