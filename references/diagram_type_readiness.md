...
| `ontology_map` | `beta` | Concept/class maps with predicates, datatype attributes, cardinality, instances, and side panels. | Use concepts for class semantics, instances only as examples, and side panels for legend/rules/version. Keep predicates short. | It can drift into either ER-table style or a generic graph if concepts, instances, and predicates are not clearly separated. | Add more ontology examples where many instances share a concept and instance lanes need heavier staggering. |
| `capability_domain_map` | `beta` | Strategic objectives, domains, sub-domains, capabilities, enablers, and sparse dependencies. | Let row/column alignment carry the meaning; keep relationship overlays sparse; use row/column header icons instead of per-card badges. | Dense dependencies can quickly become a mesh even when cards are readable. | Add a dependency-focused companion view or relationship matrix for high-link cases. |
