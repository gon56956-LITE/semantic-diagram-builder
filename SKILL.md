...
- `entities[]`: the shared row/column entity set. Each entity needs `id` and `label`; optional `subtitle`, `kind`, `accent`, and `order`.
...
- After template-level visual/layout changes, regenerate `templates/template-gallery.html` with `py scripts/build_style_gallery.py templates/template-gallery.html ...` and provide that HTML path as the human-review entry point.
- After template-level visual/layout changes, regenerate `templates/template-layout-report.md` with `py scripts/report_template_layout.py templates/template-layout-report.md` so dense, sparse, oversized, or truncation-heavy templates stay visible as follow-up candidates. Use the height diagnostics to distinguish content-driven height from real redundancy, and the truncation diagnostics to distinguish acceptable compact-fit labels from high-semantic text that needs review. Treat height above `max_height` as a review signal only; compress only when the report or visual QA shows real redundancy such as excess whitespace or low content utilization.
...
py scripts/render_semantic_diagram.py templates/relationship_matrix/reference-contract.json output.svg
